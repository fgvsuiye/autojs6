...
function killAPP(packageName){
    ...
    while (!yyxx.exists()){ 
        log("没有找到小米社区App");
        app.openAppSetting(app.getPackageName("小米社区"));
        sleep(300)
        i++;
        if(i%2===0) app.openAppSetting(packageName);
        if(i>10)break;
    }
    sleep(1000)
...
function launchAPP(packageName){
    ...
    sleep(500)
    let xmsqAPP = className('android.widget.TextView').packageName('com.xiaomi.vipaccount').text('论坛');
    let n = 0;
    while (!xmsqAPP.exists()){ 
        app.launchApp("小米社区");
        sleep(300)
        n++;
        if(n%2===0) app.launchPackage(packageName);
        if(n>5){
            log("无法打开小米社区");
            return;
        }
