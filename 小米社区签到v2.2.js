...
*/
 ...
 function launchAPP(packageName) {
     ...
     while (!xmsqAPP.exists()) {
         ...
         n++;
         if (n > 5) {
             let yyxx = className("android.widget.TextView").text("应用信息").findOne();
             let yyxq = className("android.widget.TextView").text("应用详情")
             if (yyxx) {
 ...
 function posts(n) {
     ...
     if (pkly) {
         pkly.parent().click();
         //click(textView.bounds().centerX(), textView.bounds().centerY());

     } else {
         toastLog("第" + n + "次重试")
         swipe(dwidth * 4 / 5, dheight * 3 / 4, dwidth * 4 / 5, dheight * 1 / 4, 1500);
         if (n > 3) {
             toastLog("打开帖子失败")
             return;
         }
 ...
 function 小程序签到() {
     ...
     if (v53) {
         ...
         while (!xxcx.exists()) {
             ...
             if (className("android.widget.TextView").text("通讯录").exists()) {
