...
### 4. 给予权限
- 在 AutoJs6 中，授予脚本所需的权限，如：**无障碍服务**、**悬浮窗**、**后台弹出界面**、**截图**等。
- 左侧边栏内的权限尽可能全部开启，避免运行问题。
...
签到目前依赖第三方验证码识别服务，目前仅适配**云码**识别，如需使用，请参照以下步骤操作。
1.  注册并获取验证码识别服务的Token：
    *   **云码**: [点击前往注册](https://www.jfbym.com/)
2.  根据需求充值额度，确保有足够的Token用于验证码识别。（建议充值最低档位）。
3.  进入个人中心找到我的账号中的Token。
4.  将获取到的Token填入 `config.js` 中对应的配置项。

此网站充值10元可得10000积分，目前识别价格为16积分/次。

***目前仅支持云码识别，后续会支持更多验证码识别服务。***
...
### V3.14 (260924)
 > 1. `修改` 由于验证码识别难度增加，训练模型变得异常复杂，因此替换为付费第三方验证码识别服务。
 > 2. `修改` 删除更新及本地模型文件。
### V3.13 (251107)
